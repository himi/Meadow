...
;; One of the symbols `sjis', `iso2022-7', `iso2022-8', `big5', or `utf-8' to
    ...
    (if (< head ?\x80)
	 (setq char head)
      (setq uval
	    (cond
	     ((< head ?\xe0)
	      (logior (lsh (logand head ?\x1F) 6)
		      (logand (read-char-exclusive) ?\x3F)))
	     ((< head ?\xf0)
	      (logior (lsh (logand head ?\xF) 12)
		      (lsh (logand (read-char-exclusive) ?\x3F) 6)
		      (logand (read-char-exclusive) ?\x3F)))
	     ((< head ?\xf8)
	      (logior (lsh (logand head ?\x7) 18)
		      (lsh (logand (read-char-exclusive) ?\x3F) 12)
		      (lsh (logand (read-char-exclusive) ?\x3F) 6)
		      (logand (read-char-exclusive) ?\x3F)))
	     ((< head ?\xfc)
	      (logior (lsh (logand head ?\x3) 24)
		      (lsh (logand (read-char-exclusive) ?\x3F) 18)
		      (lsh (logand (read-char-exclusive) ?\x3F) 12)
		      (lsh (logand (read-char-exclusive) ?\x3F) 6)
		      (logand (read-char-exclusive) ?\x3F)))
	     (t
	      0))
	    char (ucs-to-char uval)))
    (if (null char)
	(error "Can't handle UCS char:U+%X" uval))
    (setq unread-command-events (cons char unread-command-events))))
  ...
  (if encoded-kbd-mode
	...
	(cond ((null coding)
		...
		'use-8th-bit (nth 3 saved-input-mode))	
	      ...
	      ((and (= (coding-system-type coding) 4) ; UTF-8
