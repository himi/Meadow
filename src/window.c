...
#ifdef MEADOW
      /* Unblock input here to prevent deadlock.  It may cause
	 deadlock to call a function which handles a message in a
	 critical section.
      */
      W32_UNBLOCK_INPUT;
#endif
  ...
  if (FRAME_WINDOW_P (f))
    {
      ...
      w->vscroll = - (int) (CANON_Y_UNIT (f) * XFLOATINT (vscroll));
...
reinit_window_once (int reinit)
{
  ...
  if (reinit)
    call1 (Qface_set_after_frame_default, Vterminal_frame);
...
#ifdef PDUMP
  staticpro_nopdump (&Vwindow_list);
#else
...
The selected frame is the one whose configuration has changed.");
  ...
  DEFVAR_LISP ("select-window-functions", &Vselect_window_functions,
    "This is a hook when select-window is called.\n\
