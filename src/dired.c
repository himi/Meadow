...
/* S_ISLNK and S_IFLNK are defined in mingw-runtime, although symbolic
   link is not supported on Windows. */
#if defined(MEADOW) && defined(__MINGW32__)
#undef S_ISLNK
#undef S_IFLNK
#endif
...
#if defined (WINDOWSNT) && defined (emacs)
  /* On windows, It is necessary to inhibit quit between opendir and
     closedir. If re_search quits in this section, following opendir
     never succeed. */
  quit_not_inhibited = NILP (Vinhibit_quit);
  Vinhibit_quit = Qt;
#endif
...
#ifdef MEADOW
  if (file_attributes_stat (XSTRING (encoded)->data, &s) < 0)
    return Qnil;
#else
