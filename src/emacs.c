...
#ifdef MEADOW
  init_w32system();     /* fundamental initialization
...
#ifdef SPECIAL_EXEC
      Vtop_level = Fcons (intern ("load"),
			  Fcons (build_string ("cd-startup.el"), Qnil));
#else
...
#ifdef PDUMP_DEBUG
  {
    pdump_message ("performing GC... \n");
    Fgarbage_collect ();
    pdump_message ("performing GC... 1 \n");
    Fgarbage_collect ();
    pdump_message ("performing GC... 1 2 \n");
    Fgarbage_collect ();
    pdump_message ("performing GC... 1 2 3 done\n");
  }
#endif
...
#ifndef WINDOWSNT
  {
    printf ("copying temacs -> emacs\n");
    Fadd_name_to_file (build_string ("temacs"), build_string ("emacs"), Qt);
  }
#else  /* WINDOWSNT */
  {
    char temacs_exe[MAX_PATH];
    char emacs_exe[MAX_PATH];
    if (GetModuleFileName (NULL, temacs_exe, MAX_PATH)
	&& (xstricmp (temacs_exe + strlen (temacs_exe) - strlen ("temacs.exe"),
		      "temacs.exe")
	    == 0))		/* are you running temacs.exe ? */
      {
	memcpy (emacs_exe, temacs_exe, MAX_PATH);
	memcpy (emacs_exe + strlen (emacs_exe) - strlen ("temacs.exe"),
		"emacs.exe", strlen ("emacs.exe") + 1);
	printf ("copying %s -> %s\n", temacs_exe, emacs_exe);
	Fcopy_file (build_string (temacs_exe), build_string (emacs_exe), Qt, Qnil);
      }
    else
      {
	fatal ("Dump is allowed only for temacs.exe.\n");
      }
  }
#endif /* WINDOWSNT */
#else  /* not PDUMP */
