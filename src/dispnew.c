  ...
  if (FRAME_WINDOW_P (f))
    {
      ...
      int window_pixel_width = (int) (XFLOATINT (w->width) * CANON_X_UNIT (f));
...
#endif
#ifdef MEADOW
      /* On Meadow, main thread's GDI operations may be
	 overridden by those of GUI thread so that we should
	 call GdiFlush() here by invoking flush_display(). */
      rif->flush_display (f);
#endif
    }
  else
