...
reinit_buffer_once_1 ()
{
  ...
  buffer_permanent_local_flags[XFASTINT (buffer_local_flags.buffer_file_type)] = 1;
#endif
  /* Make this one a permanent local.  */
  buffer_permanent_local_flags[XFASTINT (buffer_local_flags.buffer_file_coding_system)] = 1;
...
init_buffer_once ()
{
...
#ifdef DOS_NT
  XSETFASTINT (buffer_local_flags.buffer_file_type, idx); ++idx;
#endif
  ...
  XSETFASTINT (buffer_local_flags.buffer_file_coding_system, idx); ++idx;
...
init_buffer ()
{
...
#ifdef USE_MMAP_FOR_BUFFERS
 {
  ...
  /* We do not use PWD on Windows because it's unreliable and
     ...
     nothing of command.com, cmd.exe and nmake.exe) */
  if (getwd (buf) == 0)
    fatal ("`getwd' failed: %s\n", buf);
#endif /* not WINDOWSNT */
