     ...
     int area;
{
  ...
  width = max (0, width); /* Remove me! This is a hack to ensure
			    to avoid negative width.  */
  ...
  if (PT >= CHARPOS (scroll_margin_pos))
      ...
      else
	{
	  ...
	  if (NUMBERP (aggressive))
	    amount_to_scroll = (int) XFLOATINT (aggressive) * height;
	}
     ...
     int just_this_one_p;
{
  ...
  *w->desired_matrix->method = 0;
  ...
  else if (current_matrix_up_to_date_p && (tem = try_window_id (w)) != 0)
    {
#if GLYPH_DEBUG
