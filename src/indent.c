...
#ifdef MEADOW
DEFUN ("back-to-previous-visible-line-start",
       Fback_to_previous_visible_line_start, 
       Sback_to_previous_visible_line_start, 0, 1, 0,
       "If position is invisible, back to beginning of preveous visible line.")
  (window)
     Lisp_Object window;
{
  struct window *w;
  ...
  while (it.selective > 0 && IT_CHARPOS (it) > BEGV)
    {
      double val;
      int opt, opt_byte;
	...
	scan_newline (PT - 1, PT_BYTE - 1, BEGV, BEGV_BYTE, -1, 0);
      ...
      val = position_indentation (PT_BYTE);
      SET_PT_BOTH (opt, opt_byte);
      
      if (val < (double)it.selective) break;

      /* back to previous line start */
...
#ifdef MEADOW
  defsubr (&Sback_to_previous_visible_line_start);
#endif
