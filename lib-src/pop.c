 ...
 *   It requires two new files, md5.c, md5.h extracted from RFC-1321.
 */
...
#define DONT_NEED_PASSWORD 0
  ...
  /*
   * Get greeting responce which may include APOP key begin with "<"
   */
  if ( pop_getline (server, &ans) < 0 )
    return (0);			/* can't get greeting message */
  if (! strncmp (ans, "-ERR", 4))
    {
      strncpy (pop_error, ans, ERROR_MAX);
  ...
  /*
   * check APOP availability and user wish
   */
  if (! (flags & POP_NO_APOP) )
    {
      /*
       * must use APOP
       */
      int keylen;
      char *keybeg, *keyend, *buf, *tail;
      struct md5_ctx context;
      unsigned char digest[16];
      int i;

      /* check APOP availability */
      if ( ! ((keybeg=strchr (ans, '<')) && (keyend=strchr (keybeg, '>')))
	   && ! (flags & POP_NO_APOP) )
	{
	  strcpy (pop_error, "APOP is not available on this pop server");
	  pop_close(server);
	  return (0);
	}
      /* prepare source string to make digest */
      keylen = keyend-keybeg+1;
      buf = malloc( keylen + strlen (password) +1);
      strncpy( buf, keybeg, keylen );
      buf[keylen] = '\0';
      strcat( buf, password );

      /* make digest */
      md5_init_ctx (&context);
      md5_process_bytes (buf, strlen (buf), &context);
      md5_finish_ctx (&context, digest);
      free (buf);
      
      /* make APOP command */
      sprintf (pop_error, "APOP %s ", username);
      tail = pop_error + strlen (pop_error);
      for (i=0; i<16; i++)
	{
	  int nibble;
	  nibble = digest[i] >>4; /* high 4 bits */
	  *tail++ = nibble + ((nibble<10)? '0': 'a'-10);
	  nibble = digest[i] & 0xF; /* low 4 bits */
	  *tail++ = nibble + ((nibble<10)? '0': 'a'-10);
	}
      *tail++ = '\0';
